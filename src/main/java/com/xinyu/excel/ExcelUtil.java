...
public class ExcelUtil {
     ...
     * @throws Exception 
     */
    public static void autoSizeColumn(Sheet sheet, Integer colIndex) {
        try {
            for (int index = 0; index < colIndex; index++) {
                ...
                for (int rowIndex = 0; rowIndex <= lastRowNum; rowIndex++) {
                    logger.info("行:" + (rowIndex + 1) + ",列:" + (index + 1));
                ...
                logger.info("第" + (index + 1) + "列宽度:columnWidth=" + columnWidth);
     ...
     *
     */
    public static CellRangeAddress getCellRangeAddress(Sheet sheet, Cell cell) throws Exception {
        int sheetMergeCount = sheet.getNumMergedRegions();
        for (int i = 0; i < sheetMergeCount; i++) {
            CellRangeAddress ca = sheet.getMergedRegion(i);
            boolean inRange = ca.isInRange(cell);
    ...
    public static String getRightTypeCell(Cell cell, int rowNum, int lineNum) throws Exception {
        // 单元格内容
        String value = "";
        logger.info("rowNum:" + rowNum + "\tlineNum:" + lineNum);
    ...
    /**
     * @Title:noney类型写入内容/100
     * @author:蒋夏生
     * @date:2019年12月12日 上午9:33:03
     * @Description:TODO 创建列
     * @param row--行
     * @param content--内容
     * @param i--第几列
     * @param style--样式
    ...
    public static void creatCell(XSSFRow row, String content, int line, XSSFCellStyle style, String type)
        throws Exception {
        logger.info("ExcelUtil的creatCell:" + row + line);
